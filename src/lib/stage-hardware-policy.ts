...
export function applyStageHardwarePolicy(stages: readonly EstimateStage[]): EstimateStage[] {
  return stages.map(stage => {
    const sourcedHardwareFigure = (stage.provenance === "reference" || stage.provenance === "multi-source" || stage.provenance === "single-source" || stage.hardwareScopeApproved === true) &&
      ((stage.powerHp !== undefined && Number.isFinite(stage.powerHp) && stage.powerHp > 0)
        || Boolean(stage.powerRangeHp?.every(value => Number.isFinite(value) && value > 0)));
