...
function extractRendered(text, url, retrievedAt) {
  ...
  if (/hybrid|mhev|phev|\bhev\b|e.tech|e.power|e.hdi/i.test(engine) && /hybrid|mhev|phev|\bhev\b|e.tech|e.power/i.test(engine)) return {url,reason:'HYBRID_APPLICATION_UNSUPPORTED'};
  const stages={};
  for(const match of clean.matchAll(/### Stage ([123])\s*\n([\s\S]*?)(?=### Stage |Contact us|$)/g)) {
    ...
    if(powers.length>=2 && torques.length>=2) stages['stage'+match[1]]={powerHp:+powers[1][1],torqueNm:+torques[1][1],
  ...
  return {id,provider:'shiftech',sourceName:'Shiftech public application table',url,retrievedAt,status:'retrieved',retrievalMethod:'page',
    ...
    identity:{brand:make,modelFamily:model,generation:yearFrom+(generationLabel?' - '+generationLabel:''),yearFrom,
