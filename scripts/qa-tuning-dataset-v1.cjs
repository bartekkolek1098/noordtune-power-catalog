...
// These seven deterministic UI fixtures use the real server normalizer/resolver.
// They are synthetic test identities, not claims about real license plates.
const synthetic = [
  ["QA0001", "kia-sportage-136", "Kia", "Sportage", 1598, 100, 2024, "Diesel", "B"],
  ["QA0002", "kia-ceed-140", "Kia", "Ceed", 1353, 103, 2019, "Benzine", "A"],
  ["QA0003", "bmw-340i-hardware", "BMW", "340i F30", 2998, 240, 2016, "Benzine", "A"],
  ["QA0004", "vag-golf-diesel", "Volkswagen", "Golf", 1598, 81, 2015, "Diesel", "B"],
  ["QA0005", "generic-petrol", "Toyota", "Yaris", 1496, 92, 2022, "Benzine", "D"],
  ["QA0006", "unsupported-ev", "Tesla", "Model 3", 0, 208, 2022, "Elektriciteit", "E"],
  ["QA0007", "ford-focus-125", "Ford", "Focus EcoBoost", 999, 92, 2017, "Benzine", "B"]
].map(([plate, id, make, model, cc, kw, year, fuel, expectedLevel]) => {
  const payload = normalizeRdwVehicle({merk:make, handelsbenaming:model, cilinderinhoud:String(cc), aantal_cilinders:id === "bmw-340i-hardware" ? "6" : id === "ford-focus-125" ? "3" : "4", datum_eerste_toelating:`${year}0101`}, [{brandstof_omschrijving:fuel, nettomaximumvermogen:String(kw)}], plate);
  delete payload.raw;
  assert.equal(payload.tuningEstimate.coverageClass, expectedLevel, `${id}: expected source path`);
  return {plate, id, payload};
});
const identities = [...plates.map(plate => ({plate, id:plate.toLowerCase()})), ...synthetic];
const cases = [
  ...[320,1440].flatMap(width => identities.map(fixture => ({...fixture, width, locale:"nl"}))),
  ...["en","pl"].flatMap(locale => [...identities.filter(f=>["V380ST","KKH27K","QA0001"].includes(f.plate))].map(f=>({...f,width:320,locale})))
];
const report = {checkedAt:new Date().toISOString(), base, method:"Four owner plates use the actual HTTP RDW route; seven additional cases use deterministic real-resolver DTOs intercepted only for those synthetic cases.", screenshots:[], results:[], errors:[]};
    ...
    for (const fixture of cases) {
      ...
      try {
        ...
        for (let index=0;index<3;index++) {
          ...
          else if (plate !== "KKH27K" || index === 0) assert.equal(quote.kind,"from");
        ...
        if (locale === "nl" && width === 320 && ["QA0001","QA0003","V380ST","KKH27K"].includes(plate)) {
          const screenshot = `${fixture.id}-${width}.png`;
          await root.screenshot({style:'header, [class~="fixed"] {visibility:hidden !important;}',path:path.join(output,screenshot)});
          report.screenshots.push(screenshot);
        }
