...
function years(label) {
  ...
  return values.length ? {yearFrom: values[0], ...(values[1] ? {yearTo: values[1]} : {})} : undefined;
}
function extract(response, scope) {
  ...
  const normalIndex = header?.findIndex(cell => /^Normal(?: tuning)?$/i.test(cell));
  ...
  const cc = Number(engine.match(/\b\d[.,]\d(?!\d)/)?.[0].replace(',', '.')) * 1000;
  ...
  const identity = {brand: scope.brand, modelFamily: scope.modelFamily, generation: scope.generation,
    ...
    powerUnit: 'PS', torqueUnit: 'Nm', electrification: /hybrid|phev|mhev|\bhev\b|e.power|e.tech|\bHSD\b|\b\d{3}h\b|\b\d{3}e\b|cooper s e\b|\bniro\b/i.test(title) ? 'hybrid' : 'unknown'};
  const stages = {};
  ...
  return {id, provider: 'unlimited-tuning', sourceName: 'Unlimited Tuning NL public Normal/Stage 2 table', url: response.url,
    ...
    notes: ['Normal is the ordinary remap observation. Xtreme is not silently substituted for Normal or Stage 2.',
